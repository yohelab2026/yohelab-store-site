...
function makeEnv({ paid = true, bucket = true } = {}) {
  ...
  if (bucket) {
    env.THEME_BUCKET = {
      async get(key) {
        if (!["bunsirube-0.3.1.zip", "bunsirube-child-0.1.1.zip"].includes(key)) return null;
        return {
          body: new Uint8Array([80, 75, 3, 4]),
          httpMetadata: { contentType: "application/zip" },
          customMetadata: { version: key === "bunsirube-child-0.1.1.zip" ? "0.1.1" : "0.3.1" },
        };
      },
    };
  }
...
try {
  ...
  assert(ok.headers.get("Content-Disposition")?.includes("bunsirube-0.3.1.zip"), "expected attachment filename");
