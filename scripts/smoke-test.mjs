...
checks.push(["blog publish falls back to a default image when none is provided", blogAdmin.includes("function effectiveEyecatchUrl") && blogAdmin.includes("ensureEyecatchFromVisibleCover") && !blogAdmin.includes("アイキャッチ画像を設定してください。") && !blogAdmin.includes("coverDrop.scrollIntoView({ behavior: 'smooth', block: 'center' });") && blogPostApi.includes('const FALLBACK_IMAGE = "/yohelab-mascot-v2-20260518.png";') && blogPostApi.includes("const eyecatch = requestedEyecatch || firstBodyImage || FALLBACK_IMAGE;") && !blogPostApi.includes('error: "eyecatch_required"') && !blogPostApi.includes('error: "invalid_eyecatch_image_url"') && !blogPostApi.includes('error: "invalid_image_url"') && !blogPostApi.includes("if (!eyecatch)") && blogPostApi.includes("sanitizeVerifiedImageUrl") && blogPostApi.includes("collectVerifiedImageUrls") && blogDraftApi.includes('const FALLBACK_IMAGE = "/yohelab-mascot-v2-20260518.png";') && blogDraftApi.includes("const eyecatch = requestedEyecatch || firstBodyImage || FALLBACK_IMAGE;") && !blogDraftApi.includes('error: "invalid_eyecatch_image_url"') && !blogDraftApi.includes('error: "invalid_image_url"') && blogImportStatic.includes("sanitizeVerifiedImageUrl") && blogImportStatic.includes("collectVerifiedImageUrls") && imageUrlGuard.includes("if (safe) urls.push(safe);") && !imageUrlGuard.includes('throw new Error("invalid_image_url")')]);
