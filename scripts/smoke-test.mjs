...
checks.push(["theme delivery points to latest package and manifest", themeDownload.includes('key: "bunsirube-0.3.5.zip"') && themeUpdate.includes('\\"version\\": \\"0.3.5\\"') && themeUpdate.includes("/lp/bunsirube/updates/") && /\\"package_sha256\\": \\"[a-f0-9]{64}\\"/.test(themeUpdate)]);
checks.push(["theme source is checked locally or intentionally absent in CI", hasThemeSource || process.env.GITHUB_ACTIONS === "true"]);
checks.push(["theme internal version matches latest package", !hasThemeSource || (themeStyle.includes("Version: 0.3.5") && themeFunctions.includes("BUNSIRUBE_VERSION', '0.3.5'") && themeSettings.includes("不具合対応の範囲") && !themeSettings.includes("ZIP導入、基本設定、記事型、文標ショートコードの使い方を対象"))]);
...
checks.push(["seo notify tools share the same discovery urls", seoNotifyApi.includes('SITE.discoveryUrls') && searchDiscoveryScript.includes('SITE.discoveryUrls') && sitemapFunction.includes('import { SITE }') && feedFunction.includes('import { SITE }') && viteConfig.includes('buildHomeJsonLd') && viteConfig.includes('buildBreadcrumbJsonLd')]);
  ...
  checks.push(["sitemap includes indexable product page and dynamic updated lastmod support", !read(dist("sitemap.xml")).includes('/products/bunsirube/') && sitemapFunction.includes("key.metadata?.updatedAt || key.metadata?.publishedAt || key.metadata?.date")]);
