...
checks.push(["dynamic blog card images use same-origin image route", blogPostFunction.includes("function cardImageUrl") && blogPostFunction.includes("return cardImageUrl(post.socialImage || post.eyecatch || FALLBACK_IMAGE);") && blogPostFunction.includes("/blog-images/") && blogPostFunction.includes("og:image:secure_url") && blogPostFunction.includes("twitter:image:alt") && blogPostPageSource.includes("function cardImageUrl") && blogPostPageSource.includes("return cardImageUrl(post.socialImage || post.eyecatch || FALLBACK_IMAGE);") && blogPostPageSource.includes("og:image:secure_url") && blogImageFunction.includes("onRequestHead") && blogImageFunction.includes("headOnly ? null : entry.body") && blogCardImageFunction.includes("onRequestHead") && blogCardImageFunction.includes("BLOG_IMAGES") && blogCardImageFunction.includes("X-Robots-Tag") && blogCardImageFunction.includes("Access-Control-Allow-Origin")]);
...
checks.push(["blog publish requires eyecatch but recovers visible cover image", blogAdmin.includes("アイキャッチ画像を設定してください。") && blogAdmin.includes("coverDrop.scrollIntoView") && blogAdmin.includes("function effectiveEyecatchUrl") && blogAdmin.includes("ensureEyecatchFromVisibleCover") && blogPostApi.includes('error: "eyecatch_required"') && blogPostApi.includes("if (!eyecatch)") && blogPostApi.includes("blog-images") && blogDraftApi.includes("blog-images")]);
checks.push(["future blog posts use cover image for social cards", blogAdmin.includes("function coverSocialImageUrl()") && blogAdmin.includes("socialImage: coverSocialImageUrl() || undefined") && blogPostApi.includes("const eyecatch = requestedEyecatch || requestedSocialImage || firstBodyImage") && blogDraftApi.includes("const eyecatch = requestedEyecatch || requestedSocialImage || firstBodyImage") && blogImportStatic.includes("socialImage: normalized.socialImage") && blogPostFunction.includes("hydrateStaticPostImages(post)") && blogPostsApi.includes("staticPostMetaFor") && staticBlogSocialPostsModule.includes("claude-mythos-preview") && blogPostFunction.includes('<meta name="twitter:image" content="${escAttr(eyecatchAbs)}"') && blogPostPageSource.includes("setMeta('twitter:image', imageUrl)")]);
...
const robotsTxt = read(dist("robots.txt"));
  checks.push(["robots.txt blocks admin api pro games and affiliate dashboard for all crawler groups", robotsTxt.includes('User-agent: *') && robotsTxt.includes('User-agent: GPTBot') && robotsTxt.includes('User-agent: Claude-Web') && robotsTxt.includes('User-agent: PerplexityBot') && robotsTxt.includes('User-agent: Googlebot') && robotsTxt.includes('User-agent: Bingbot') && robotsTxt.includes('User-agent: DuckDuckBot') && robotsTxt.includes('User-agent: Applebot') && robotsTxt.includes('User-agent: YandexBot') && (robotsTxt.match(/Disallow: \/blog\/admin\//g)?.length || 0) >= 9 && (robotsTxt.match(/Disallow: \/api\//g)?.length || 0) >= 9 && (robotsTxt.match(/Disallow: \/pro\//g)?.length || 0) >= 9 && (robotsTxt.match(/Disallow: \/games\//g)?.length || 0) >= 9 && (robotsTxt.match(/Disallow: \/affiliate\/dashboard\//g)?.length || 0) >= 9 && robotsTxt.includes('Sitemap: https://yohelab.com/sitemap.xml') && robotsTxt.includes('Sitemap: https://yohelab.com/feed.xml') && middleware.includes('Sitemap: https://yohelab.com/feed.xml') && middleware.includes('User-agent: Bingbot') && middleware.includes('max-age=300')]);
