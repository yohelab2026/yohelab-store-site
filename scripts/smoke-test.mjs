...
checks.push(["dynamic blog card images use same-origin image route", blogPostFunction.includes("function cardImageUrl") && blogPostFunction.includes("/blog-images/") && blogPostFunction.includes("og:image:secure_url") && blogPostFunction.includes("twitter:image:alt") && blogPostPageSource.includes("function cardImageUrl") && blogPostPageSource.includes("og:image:secure_url") && blogCardImageFunction.includes("onRequestHead") && blogCardImageFunction.includes("BLOG_IMAGES") && blogCardImageFunction.includes("X-Robots-Tag") && blogCardImageFunction.includes("Access-Control-Allow-Origin")]);
...
checks.push(["site SEO and mobile head polish is injected", viteConfig.includes("og:site_name") && viteConfig.includes("og:url") && viteConfig.includes("format-detection") && viteConfig.includes("text-size-adjust") && viteConfig.includes("touch-action: manipulation") && viteConfig.includes("min-height: 44px") && blogPostFunction.includes("isPartOf") && blogPostFunction.includes("og:image:width") && blogPostFunction.includes("article:author")]);
...
checks.push(["future blog posts use cover image for social cards", blogAdmin.includes("function coverSocialImageUrl()") && blogAdmin.includes("socialImage: coverSocialImageUrl() || undefined") && blogPostApi.includes("const socialImage = sanitizeUrl(body?.socialImage) || eyecatch") && blogDraftApi.includes("const socialImage = sanitizeUrl(body?.socialImage) || eyecatch") && blogImportStatic.includes("socialImage: normalized.socialImage") && blogPostFunction.includes("hydrateStaticPostImages(post)") && blogPostsApi.includes("staticPostMetaFor") && staticBlogSocialPostsModule.includes("claude-mythos-preview") && blogPostFunction.includes('<meta name="twitter:image" content="${escAttr(eyecatchAbs)}"') && blogPostPageSource.includes("setMeta('twitter:image', imageUrl)")]);
