...
checks.push(["blog category tabs respect selected child categories", blogMain.includes("data-category-keys") && blogMain.includes("categoryKeysForPost(post)") && blogMain.includes("return categoryKeysFromTags(post?.tags || []).slice(0, 4)") && blogMain.includes("return keys.some(key => categoryParentByKey[key] === parent)") && blogMain.includes("if (requiredParent) return keys.includes(normalized)")]);
