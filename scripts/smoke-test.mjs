...
checks.push(["blog publish falls back to a default image when none is provided", blogAdmin.includes("function effectiveEyecatchUrl") && blogAdmin.includes("ensureEyecatchFromVisibleCover") && !blogAdmin.includes("アイキャッチ画像を設定してください。") && !blogAdmin.includes("coverDrop.scrollIntoView({ behavior: 'smooth', block: 'center' });") && blogPostApi.includes('const FALLBACK_IMAGE = "/yohelab-mascot-v2-20260518.png";') && blogPostApi.includes("const eyecatch = requestedEyecatch || firstBodyImage || FALLBACK_IMAGE;") && !blogPostApi.includes('error: "eyecatch_required"') && !blogPostApi.includes("if (!eyecatch)") && blogPostApi.includes("sanitizeVerifiedImageUrl") && blogPostApi.includes("collectVerifiedImageUrls") && blogPostApi.includes("invalid_eyecatch_image_url") && blogDraftApi.includes("sanitizeVerifiedImageUrl") && blogDraftApi.includes("collectVerifiedImageUrls") && blogDraftApi.includes("invalid_eyecatch_image_url") && blogImportStatic.includes("sanitizeVerifiedImageUrl") && blogImportStatic.includes("collectVerifiedImageUrls")]);
